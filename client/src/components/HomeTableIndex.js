...
class HomeTable extends Component {
  ...
  markTodayEmployee(index, label){
    ...
    axios.get("/api/v1/employee/mark/"+this.state.data[index]._id+"/"+date+"/"+label)
    .then(
      (res) => {    
  ...
  markTodayAllEmployees(label){
    ...
    var date = this.state.today;
    axios.get("/api/v1/employee/all/mark/"+date+"/"+label)
    .then(
      (res) => {    
        alert('Recorded successfully!');
        var temp = this.state.data;
    ...
    if(!this.state.loading)
      return (
        <div className="container">
          ...
          <div className="table-controls justify-center align-stretch">
            <div>
              <Button className="btn" title="Go back in time"
            ...
            <div>
              <Button className="btn" title="Go forward in time"
                onClick={() => this.addDateOffset(1) }>
                &gt;
              </Button>
            </div>
